...
import { loadBranches, selectBranch, selectRepo } from "@/webview/lib/actions";
...
function setShowRemoteBranch(newValue: boolean) {
  showRemoteBranch.value = newValue;
  loadBranches();
}
...
export function MainHeader({ repos }: { repos: Array<RepoData> }) {
  return (
    <header class="flex flex-wrap items-center justify-center gap-x-8 gap-y-4 border-b border-line py-4">
      <Button aria-label={window.l10n.refresh} title={window.l10n.refresh} onClick={loadBranches}>
        <Icon class="">
